...
import EventPublisher from '../usecase/EventPublisher';

class IPC implements EventPublisher {
  constructor(
    private readonly window: BrowserWindow,
    private readonly interactor: DashboardInteractor,
  ) {
  }
  ...
  registerHandlers() {
    ...
    ipcMain.on(Events.REQ_ADD_WIDGET, (evt, message: Message) => {
      ...
      this.interactor.addWidget(widget)
        .then(_ => evt.reply(Events.RES_OK_ADD_WIDGET))
    ...
    ipcMain.on(Events.REQ_EDIT_WIDGET, (evt, message: Message) => {
      ...
      this.interactor.updateWidget(updated.id, updated)
        .then(_ => evt.reply(Events.RES_OK_EDIT_WIDGET))
    ...
    ipcMain.on(Events.REQ_DELETE_WIDGET, (evt, message: Message) => {
      ...
      this.interactor.deleteWidget(id)
        .then(_ => evt.reply(Events.RES_OK_DELETE_WIDGET))
